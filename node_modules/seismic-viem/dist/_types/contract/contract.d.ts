...
import type { Abi, Account, Address, Chain, Client, ContractFunctionName, GetContractParameters, GetContractReturnType, IsNarrowable, IsNever, Transport, WriteContractReturnType } from 'viem';
import type { ShieldedWalletClient } from '../client.ts';
import { ShieldedWriteContractDebugResult } from '../contract/write.ts';
import type { KeyedClient } from '../viem-internal/client.ts';
import type { GetReadFunction, GetWriteFunction } from '../viem-internal/function.ts';
type TransparentReadContractReturnType<TAbi extends Abi | readonly unknown[], TClient extends Client | KeyedClient = Client | KeyedClient, _readFunctionNames extends string = TAbi extends Abi ? Abi extends TAbi ? string : ExtractAbiFunctionNames<TAbi, 'pure' | 'view'> : string, _narrowable extends boolean = IsNarrowable<TAbi, Abi>, _publicClient extends Client | unknown = TClient extends {
    public: Client;
} ? TClient['public'] : TClient> = _publicClient extends Client ? IsNever<_readFunctionNames> extends true ? unknown : {
    tread: {
...
type TransparentWriteContractReturnType<TAbi extends Abi | readonly unknown[], TClient extends Client | KeyedClient = Client | KeyedClient, _writeFunctionNames extends string = TAbi extends Abi ? Abi extends TAbi ? string : ExtractAbiFunctionNames<TAbi, 'nonpayable' | 'payable'> : string, _narrowable extends boolean = IsNarrowable<TAbi, Abi>, _walletClient extends Client | unknown = TClient extends {
    ...
    twrite: {
        [functionName in _writeFunctionNames]: GetWriteFunction<_narrowable, _walletClient['chain'], _walletClient['account'], TAbi, functionName extends ContractFunctionName<TAbi, 'nonpayable' | 'payable'> ? functionName : never, WriteContractReturnType>;
    };
    dwrite: {
        [functionName in _writeFunctionNames]: GetWriteFunction<_narrowable, _walletClient['chain'], _walletClient['account'], TAbi, functionName extends ContractFunctionName<TAbi, 'nonpayable' | 'payable'> ? functionName : never, ShieldedWriteContractDebugResult<_walletClient['chain'], _walletClient['account']>>;
    };
} : unknown;
/**
