...
__export(exports_src, {
  ...
  getSeismicClients: () => getSeismicClients,
...
var { bytesToNumberBE: b2n2, hexToBytes: h2b2 } = exports_utils2;
...
function weierstrass2(curveDef) {
  ...
  function verify(signature, msgHash, publicKey, opts = defaultVerOpts) {
    ...
    if (format !== undefined && format !== "compact" && format !== "der")
      throw new Error("format must be compact or der");
    const isHex2 = typeof sg === "string" || isBytes6(sg);
    const isObj = !isHex2 && !format && typeof sg === "object" && sg !== null && typeof sg.r === "bigint" && typeof sg.s === "bigint";
    if (!isHex2 && !isObj)
      throw new Error("invalid signature, expected Uint8Array, hex string or Signature instance");
    let _sig = undefined;
...
function extract2(hash2, ikm, salt) {
  ...
  return hmac2(hash2, toBytes4(salt), toBytes4(ikm));
}
...
function expand(hash2, prk, info, length = 32) {
  ...
  const HMAC3 = hmac2.create(hash2, prk);
...
var sharedSecretPoint = ({
  ...
  return secp256k12.getSharedSecret(privateKeyHex, networkPublicKey, false).slice(1);
};
var sharedKeyFromPoint = (sharedSecret) => {
  const version4 = sharedSecret[63] & 1 | 2;
  const finalSecret = sha2564.create().update(new Uint8Array([version4])).update(sharedSecret.slice(0, 32)).digest();
...
var deriveAesKey = (sharedSecret) => {
  const derivedKey = hkdf(sha2564, hexToBytes(`0x${sharedSecret}`), new Uint8Array(0), new TextEncoder().encode("aes-gcm key"), 32);
...
function shouldPerformMulticall2({
  request
}) {
  const { data, to, ...request_ } = request;
  if (!data)
...
class InvalidBytesLengthError2 extends BaseError2 {
  constructor({
    size: size4,
    targetSize,
    type
...
function createBatchScheduler2({
  fn,
  id,
  shouldSplitBatch,
  wait: wait2 = 0,
...
async function signedCall(client, args) {
  ...
  try {
    ...
    const blockNumberHex = blockNumber ? numberToHex(blockNumber) : undefined;
    const block = blockNumberHex || blockTag;
    const rpcStateOverride = serializeStateOverride2(stateOverride);
    const chainFormat = client.chain?.formatters?.transactionRequest?.format;
...
async function sendShieldedTransaction(client, parameters) {
  ...
  try {
    ...
    if (account?.type === "json-rpc" || account === null || account?.type === "local") {
      let chainId;
      if (chain !== null) {
        chainId = await getAction(client, getChainId, "getChainId")({});
        assertCurrentChain({
      ...
      const preparedTx = await prepareTransactionRequest(client, request);
      const serializedTransaction = await account.signTransaction({ seismicInput, encryptionPubkey, ...preparedTx }, { serializer: serializeSeismicTransaction });
...
var compressPublicKey = (uncompressedKey) => {
  ...
  const pt = secp256k12.ProjectivePoint.fromHex(cleanKey);
...
var createShieldedPublicClient = async (parameters) => {
...
var getSeismicClients = async ({
  ...
  const publicClient = await createShieldedPublicClient({ chain, transport, encryptionSk });
  ...
  const wallet = createClient({ account, chain, transport }).extend(publicActions).extend(walletActions).extend(() => encryptionActions(aesKey, encryptionPublicKey)).extend(shieldedPublicActions).extend(shieldedWalletActions);
  return {
    public: publicClient,
    wallet
  };
