...
__export(exports_src, {
  ...
  signSeismicTxTypedData: () => signSeismicTxTypedData,
  shieldedWriteContractDebug: () => shieldedWriteContractDebug,
  shieldedWriteContract: () => shieldedWriteContract,
  sharedSecretPoint: () => sharedSecretPoint,
  sharedKeyFromPoint: () => sharedKeyFromPoint,
  ...
  aesGcmDecryptPrecompile: () => aesGcmDecryptPrecompile,
...
var seismicRpcSchema = [estimateGasRpcSchema, callRpcSchema];
    ...
    format: (request) => {
      ...
      if (request.encryptionPubkey) {
        ...
        if (request.messageVersion && request.messageVersion !== 0) {
          throw new Error("Message version must be 0 for seismic transaction requests");
        }
        seismicElements = {
...
var signSeismicTxTypedData = async (client, tx) => {
...
function getWebcryptoSubtle() {
  if (crypto3 && typeof crypto3.subtle === "object" && crypto3.subtle != null)
    return crypto3.subtle;
...
// src/crypto/nonce.ts
var randomEncryptionNonce = () => bytesToHex(randomBytes2(12));
...
async function signedReadContract(client, parameters) {
  ...
  const encryptedCalldata = await aesCipher.encrypt(plaintextCalldata, encryptionNonce);
  const request = {
    ...rest,
    nonce,
    to: address,
...
var getPlaintextCalldata = (parameters) => {
  const { abi: abi2, functionName, args = [] } = parameters;
  const seismicAbi = getAbiItem({ abi: abi2, name: functionName });
  const selector = toFunctionSelector(formatAbiItem2(seismicAbi));
  const ethAbi = remapSeismicAbiInputs(seismicAbi);
  const encodedParams = encodeAbiParameters(ethAbi.inputs, args).slice(2);
...
async function getShieldedWriteContractRequest(client, parameters) {
...
// src/explorer.ts
var getExplorerUrl = (chain, options) => {
  if (!chain) {
    return null;
  }
...
var callPrecompile = async ({
  client,
  precompile,
  args
}) => {
  const data = precompile.encodeParams(args);
  const gas = BASE_TX_GAS_COST + calldataGasCost(data) + precompile.gasCost(args);
...
var AES_GCM_ENCRYPT_ADDRESS = "0x0000000000000000000000000000000000000066";
var AES_GCM_DECRYPT_ADDRESS = "0x0000000000000000000000000000000000000067";
var AES_GCM_BASE_GAS = 1000n;
var AES_GCM_PER_BLOCK = 30n;
var MIN_AES_PLAINTEXT_LENGTH = 0;
var MIN_AES_CIPHERTEXT_LENGTH = 16;
var aesGcmGasCost = (value) => {
  const valueBytes = isHex(value) ? hexToBytes(value) : stringToBytes(value);
...
var validateParams = (args, encryption) => {
  ...
  const nonceBytes = numberToBytes(nonce, { size: 12 });
  if (nonceBytes.length !== 12) {
    throw new Error(`Invalid nonce: expected 12 bytes but found ${nonceBytes.length}`);
  }
  if (encryption) {
    const plaintextBytes = stringToBytes(input);
    ...
    const ciphertextBytes = hexToBytes(input);
  ...
  return [aesKey, BigInt(nonce), input];
};
var aesGcmEncryptPrecompile = {
  ...
  decodeResult: (result) => trim(result)
};
var aesGcmDecryptPrecompile = {
  ...
  decodeResult: (result) => hexToString(trim(result))
};
...
var aesGcmDecrypt = async (client, args) => {
  return callPrecompile({
    client,
...
var HKDF_EXPAND_COST_GAS = 2n * SHA256_BASE_GAS;
var SHARED_SECRET_GAS = 3000n;
var hdfkPrecompile = {
...
var ECDH_ADDRESS = "0x0000000000000000000000000000000000000065";
...
var persToBytes = (pers) => {
  if (!pers) {
    return new Uint8Array;
  }
  if (isHex(pers)) {
    return hexToBytes(pers);
  } else if (isBytes3(pers)) {
    return pers;
  }
...
var shieldedPublicActions = (client) => ({
  ...
  addressExplorerUrl: (options) => addressExplorerUrl({ chain: client.chain, ...options }),
  blockExplorerUrl: (options) => blockExplorerUrl({ chain: client.chain, ...options }),
  txExplorerUrl: (options) => txExplorerUrl({ chain: client.chain, ...options }),
  tokenExplorerUrl: (options) => tokenExplorerUrl({ chain: client.chain, ...options }),
  rng: async (args) => callPrecompile({
    client,
    precompile: rngPrecompile,
    args
  }),
  ecdh: (args) => callPrecompile({
    client,
    precompile: ecdhPrecompile,
    args
  }),
  hdfk: (input) => callPrecompile({
    client,
    precompile: hdfkPrecompile,
    args: input
  }),
  secp256k1Signature: (args) => callPrecompile({
    client,
    precompile: secp256k1SigPrecompile,
    args
  }),
...
// src/faucet.ts
var DEFAULT_MIN_BALANCE_WEI = parseEther("0.5");
...
var checkFaucet = async ({
  ...
  if (msg.startsWith("Txhash: ")) {
    const hash2 = msg.slice(8);
    if (hash2.startsWith("0x") && hash2.length === 66) {
      const txUrl = txExplorerUrl({ chain: publicClient.chain, txHash: hash2 });
      if (txUrl) {
        console.debug(`Faucet sent eth to ${address}: ${txUrl}`);
      }
      await publicClient.waitForTransactionReceipt({ hash: hash2 });
      return { sent: true, hash: hash2, txUrl: txUrl ?? undefined };
    } else {
      throw new Error(`Invalid hash from faucet claim: ${hash2}`);
    }
  }
